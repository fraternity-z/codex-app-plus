...
pub(crate) fn proxy_environment_assignments(settings: &ProxySettings) -> Vec<ProxyEnvironmentEdit> {
    match settings.mode {
...
pub(crate) fn apply_std_proxy_environment(command: &mut StdCommand, settings: &ProxySettings) {
...
mod tests {
    ...
    fn removes_proxy_environment_from_std_command_when_disabled() {
        ...
        #[cfg(windows)]
        assert!(
            matches!(env_map.get("http_proxy"), Some(None) | None),
            "Windows Command environment edits are case-insensitive"
        );
    }
    ...
    fn parses_single_windows_proxy_as_http_and_https_proxy() {
        let values = system_proxy_values_from_windows_proxy_config("127.0.0.1:7890", None);
