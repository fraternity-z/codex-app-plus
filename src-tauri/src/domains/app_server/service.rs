...
use super::bundled_browser_use;
use super::bundled_computer_use;
use super::mcp_shared_pool::{self, McpSharedPoolRuntime};
use super::models::{
...
fn configure_browser_use_iab_environment(agent_environment: AgentEnvironment, cli: &mut CodexCli) {
    ...
    if agent_environment == AgentEnvironment::WindowsNative {
        if let Some(pipe_name) = crate::domains::browser::use_backend::iab_pipe_name() {
            cli.environment.push((
                crate::domains::browser::use_backend::IAB_PIPE_ENV.to_string(),
