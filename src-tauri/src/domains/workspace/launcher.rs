...
use super::models::{OpenFileInEditorInput, OpenWorkspaceInput, WorkspaceOpener};
use crate::error::{AppError, AppResult};
use crate::infra::filesystem::agent_environment::{
    resolve_agent_environment, resolve_host_path_for_agent_path,
};
use crate::infra::process::command::{
    open_detached_target, spawn_background_command, spawn_hidden_background_command,
};
use crate::infra::process::windows_child::configure_visible_console_std_command;
...
pub fn open_file_in_editor(input: OpenFileInEditorInput) -> AppResult<()> {
    ...
    let spec =
        build_open_file_command_spec_with(&binary, &input, resolve_host_path_for_agent_path)?;
