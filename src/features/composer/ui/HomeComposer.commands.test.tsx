...
describe("HomeComposer commands", () => {
  ...
  it("routes /goal through the send path so the controller can create or update the goal", async () => {
    const onSendTurn = vi.fn().mockResolvedValue(undefined);
    const request = vi.fn().mockResolvedValue({});
    renderHarness({ onSendTurn, request });
    const textarea = screen.getByRole("textbox");
    ...
    expect(request).not.toHaveBeenCalledWith("thread/goal/set", expect.anything());
  });
